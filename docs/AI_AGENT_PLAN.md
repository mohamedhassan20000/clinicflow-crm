...
| **P4.6 — Staff Analytics & Financial Assistant** (`pro_ai`-only; post-v1, non-blocking) | 7–10 | Read-only operational + financial analytics tools (`get_clinic_summary`, `get_revenue_summary`, report lookups) on purpose-built `SECURITY DEFINER` clinic-scoped aggregate RPCs; `ai.financial_insights` entitlement + per-user financial permission (default OFF for managers); receptionists/doctors excluded from financial tools | P4, P4.5 |
...
| **Total** | **~135–191** (≈ 9–12 months, single developer) | | |
...
| P4.6 | **P4.6A** | Aggregate analytics/financial RPCs (`SECURITY DEFINER`, clinic-scoped) + `ai.financial_insights` entitlement + per-user financial permission + per-tool authorization + tool-authorization tests (no UI) | P4.5A, P4.5C | 4–6 | `feat/p46a-staff-analytics-tools` |
| P4.6 | **P4.6B** | Analytics/financial tools mounted into the existing staff assistant UI + entitlement/permission-gated + upgrade/permission UX | P4.6A, P4B | 3–4 | `feat/p46b-staff-analytics-ui` |
...
### P4.6 — Staff Analytics & Financial Assistant (7–10 days) — *added 2026-07-19; `pro_ai`-only; post-v1, non-blocking*
...
- **Goal:** give admins (and explicitly permission-granted managers) a **read-only** analytical assistant for operational and financial insight, reusing the same authorization spine as P4 — never a generic query tool.
- **New tools (specialized, read-only, per-persona, allow-listed):**
  - `get_clinic_summary` — operational aggregates (patient / appointment / follow-up counts and trends). **No PHI, no financials.**
  - `get_revenue_summary` — financial aggregates (revenue, outstanding, deposits) at the aggregate level only.
  - Report-oriented lookups scoped to the existing Reports module surfaces.
  - Each is backed by a **purpose-built `SECURITY DEFINER`, clinic-scoped aggregate RPC that returns aggregates only** — never a raw-row scan through RLS — mirroring the `log_agent_tool_call` service-boundary pattern. This keeps the AI off large table scans and returns exactly the shape the tool needs.
- **Role/tool matrix for this phase** *(this is P4.6's own matrix and does not modify the shipped §6.8 P4 matrix):*

  | Role | Operational analytics (`get_clinic_summary`, report lookups) | Financial analytics (`get_revenue_summary`) |
  |---|---|---|
  | Admin | Yes | Yes |
  ...
  | Receptionist | No (not mounted) | No (not mounted) |
  | Doctor | No (clinic-wide analytics out of scope) | No |
...
**P4.6A — Aggregate RPCs + entitlement/permission plumbing** — branch `feat/p46a-staff-analytics-tools`, est. **4–6 days**, merge **1st**, **no UI**.
- *In scope:* `SECURITY DEFINER` clinic-scoped aggregate RPCs; `get_clinic_summary` / `get_revenue_summary` / report-lookup tools behind the P4.5 policy boundary; `ai.staff_analytics` + `ai.financial_insights` namespaced entitlements; per-user financial permission storage + admin management; `assertFinancialInsightsAccess`; deny-by-default tool registration; the tool-authorization test suite (the security-critical deliverable, reviewed without UI dilution).
- *Out:* any chat UI (P4.6B); patient analytics.
- *Acceptance:* every matrix cell above is enforced by tests; no financial tool is mountable without entitlement + permission; RPCs return aggregates only and never cross clinics.

**P4.6B — Analytics tools in the staff assistant UI** — branch `feat/p46b-staff-analytics-ui`, est. **3–4 days**, merge **2nd**.
- *In scope:* mount the P4.6A tools into the existing P4B staff assistant surface; entitlement/permission-gated affordances; upgrade/permission-denied UX copy (ar/en); operational-vs-financial result presentation.
- *Out:* new standalone analytics pages; write actions.
- *Acceptance:* an entitled admin can ask for operational and financial summaries in chat; a manager without the permission sees a clear, localized "not enabled" state, never a financial answer.
