-- Phase 4: extend appointment-lifecycle RPC authorization to Managers.
--
-- These functions are reproduced verbatim from
-- 20260519002500_in_visit_transitions_undo_and_slots.sql; the ONLY change is
-- adding 'manager' to the operational-role guard so a Manager can complete/undo
-- and change appointment status exactly like an admin/receptionist. Existing
-- grants are preserved by CREATE OR REPLACE. Financial settlement of standalone
-- patient outstanding balances (settle_patient_outstanding) is intentionally
-- NOT changed here.
...
begin
  ...
  if v_role <> all (array['admin'::public.user_role, 'receptionist'::public.user_role, 'manager'::public.user_role]) then
