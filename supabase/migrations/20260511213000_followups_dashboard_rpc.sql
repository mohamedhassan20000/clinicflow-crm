-- Performance Phase 2B: DB-backed follow-up lists and summary counts.
-- SECURITY INVOKER is intentional so existing appointment/follow_up/patient RLS
-- remains in force for each caller role.
...
begin
  ...
  if v_role = 'manager'::public.user_role then
    raise exception 'Not authorized to view follow-ups'
      using errcode = '42501';
  end if;
  ...
  done_scope as (
    ...
    where f.clinic_id = v_clinic_id
      and f.recorded_at >= p_start
      and f.recorded_at <= p_end
