-- P7 Phase 4: canonical reprint transaction for the generic free-form
-- ("Create document from scratch") document type. Mirrors the P7-5 roster
-- reprint contract but is restricted to the single GENERIC_DOCUMENT type and the
-- scoped-operational role set that may author it.

create or replace function public.record_generic_document_reprint(
  p_document_id uuid
)
returns table (
...
begin
  ...
  where d.id = p_document_id
    ...
    and d.doc_type = 'GENERIC_DOCUMENT'
...
alter function public.record_generic_document_reprint(uuid) owner to postgres;
revoke all on function public.record_generic_document_reprint(uuid) from public, anon;
grant execute on function public.record_generic_document_reprint(uuid) to authenticated;

comment on function public.record_generic_document_reprint(uuid) is
  'P7 Phase 4 canonical reprint transaction restricted to the GENERIC_DOCUMENT type.';
