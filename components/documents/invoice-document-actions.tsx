...
export function InvoiceDocumentActions({
  ...
  function issue() {
    startIssue(async () => {
      const result = await issueInvoiceReportDocument({ appointmentId, locale, draftId });
      if (!result.data) {
  ...
  function reprint() {
    ...
    startReprint(async () => {
      const result = await reprintInvoiceDocument(documentId);
      if (!result.data) {
