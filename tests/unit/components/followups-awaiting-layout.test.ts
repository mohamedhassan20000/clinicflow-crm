...
describe("awaiting follow-up layout", () => {
  it("keeps file number and national ID in separate non-wrapping columns", () => {
    ...
    expect(source).toContain('t("file")');
    expect(source).toContain('t("nationalId")');
    expect(messages.followups.file).toBe("File #");
    expect(messages.followups.nationalId).toBe("National ID");
