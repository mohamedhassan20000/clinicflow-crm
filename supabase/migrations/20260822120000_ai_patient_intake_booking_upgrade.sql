-- ClinicFlow AI WhatsApp patient intake + appointment review upgrade.
-- Unknown senders are staged for a human review; they are never inserted into
-- public.patients by an automated actor. Existing patients continue to be
-- linked only through the exact, phone-bound identity boundary.
...
alter table public.audit_logs
  ...
  check (actor_type is null or actor_type in ('user', 'ai', 'system'));

create or replace function public.classify_audit_actor()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  if new.actor_type is null then
    new.actor_type := case
      ...
      when new.action like 'AI\_%' escape '\' then 'ai'
      else 'system'
    end;
  end if;
  if new.source is null and new.actor_type = 'ai' then
    new.source := 'ai_assistant';
  end if;
  return new;
end;
$$;

drop trigger if exists trg_audit_logs_classify_actor on public.audit_logs;
create trigger trg_audit_logs_classify_actor
  before insert or update of actor_id, actor_type, action, source
  on public.audit_logs
  for each row execute function public.classify_audit_actor();

revoke all on function public.classify_audit_actor()
  from public, anon, authenticated, service_role;

-- Departments predate the tenant-composite FK convention used by newer
-- clinical tables. The primary key already makes id unique; this companion key
-- lets child rows enforce that their denormalized clinic_id agrees as well.
create unique index if not exists departments_id_clinic_ai_intake_unique_idx
  on public.departments (id, clinic_id);

create table public.ai_patient_intakes (
  ...
  national_id_folded text generated always as (
    public.fold_national_id(national_id)
  ) stored,
  department_id uuid not null,
  doctor_id uuid not null,
  source text not null default 'ai_assistant' check (source = 'ai_assistant'),
  review_status text not null default 'pending_review'
    check (review_status in ('pending_review', 'approved', 'rejected', 'dismissed')),
  ...
  constraint ai_patient_intakes_conversation_clinic_fkey
    foreign key (conversation_id, clinic_id)
    references public.conversations(id, clinic_id) on delete restrict,
  constraint ai_patient_intakes_department_clinic_fkey
    foreign key (department_id, clinic_id)
    references public.departments(id, clinic_id) on delete restrict,
  constraint ai_patient_intakes_doctor_clinic_fkey
    foreign key (doctor_id, clinic_id)
    references public.profiles(id, clinic_id) on delete restrict,
  constraint ai_patient_intakes_reviewer_clinic_fkey
    foreign key (reviewed_by, clinic_id)
    references public.profiles(id, clinic_id) on delete restrict,
  constraint ai_patient_intakes_patient_clinic_fkey
    foreign key (approved_patient_id, clinic_id)
    references public.patients(id, clinic_id) on delete restrict,
  constraint ai_patient_intakes_review_shape_check check (
    (review_status = 'pending_review'
      and reviewed_at is null and reviewed_by is null and approved_patient_id is null)
    or
    (review_status <> 'pending_review' and reviewed_at is not null and reviewed_by is not null)
  )
);

create unique index ai_patient_intakes_conversation_unique
  on public.ai_patient_intakes (clinic_id, conversation_id);
create unique index ai_patient_intakes_id_clinic_unique
  on public.ai_patient_intakes (id, clinic_id);
create index ai_patient_intakes_review_queue_idx
  on public.ai_patient_intakes (clinic_id, review_status, created_at desc);
...
create table public.ai_appointment_requests (
  ...
  duration_minutes integer not null default 30
    check (duration_minutes between 15 and 240 and mod(duration_minutes, 15) = 0),
  status text not null default 'pending'
    check (status in ('pending', 'linked', 'dismissed', 'expired')),
  appointment_id uuid,
  expires_at timestamptz not null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint ai_appointment_requests_intake_clinic_fkey
    foreign key (intake_id, clinic_id)
    references public.ai_patient_intakes(id, clinic_id) on delete restrict,
  constraint ai_appointment_requests_conversation_clinic_fkey
    foreign key (conversation_id, clinic_id)
    references public.conversations(id, clinic_id) on delete restrict,
  constraint ai_appointment_requests_department_clinic_fkey
    foreign key (department_id, clinic_id)
    references public.departments(id, clinic_id) on delete restrict,
  constraint ai_appointment_requests_doctor_clinic_fkey
    foreign key (doctor_id, clinic_id)
    references public.profiles(id, clinic_id) on delete restrict,
  constraint ai_appointment_requests_appointment_clinic_fkey
    foreign key (appointment_id, clinic_id)
    references public.appointments(id, clinic_id) on delete restrict,
  constraint ai_appointment_requests_status_shape_check check (
    (status = 'linked' and appointment_id is not null)
    or (status <> 'linked' and appointment_id is null)
  )
);

create unique index ai_appointment_requests_one_active_intake
  on public.ai_appointment_requests (clinic_id, intake_id)
  where status = 'pending';
create index ai_appointment_requests_review_queue_idx
  on public.ai_appointment_requests (clinic_id, status, scheduled_at);

-- The reviewed intake flow supersedes the older service-role RPC which wrote a
-- normal patient immediately. Dropping it makes the human-review invariant a
-- database fact, not a caller convention.
drop function if exists public.register_patient_from_conversation(
  uuid, uuid, text, text, date, text
);

alter table public.ai_patient_intakes enable row level security;
...
-- Maintenance companion to the insert-time transition above. The existing
-- daily booking-expiry job calls this service-only boundary so dashboard state
-- (`pending` and unexpired) and stored lifecycle state converge even when no
-- patient retries after expiry.
create or replace function public.expire_ai_appointment_requests(
...
begin
  ...
  if p_now is null or p_limit < 1 or p_limit > 10000 then
    raise exception 'AI_APPOINTMENT_REQUEST_EXPIRY_INVALID' using errcode = '22023';
  end if;
...
revoke all on function public.expire_ai_appointment_requests(timestamptz, integer)
  from public, anon, authenticated;
grant execute on function public.expire_ai_appointment_requests(timestamptz, integer)
  to service_role;

-- A delayed existing-patient booking is blocked at the database boundary after
-- a staff member takes over, closing the check-then-insert race.
create or replace function public.block_paused_ai_booking_insert()
returns trigger language plpgsql set search_path = '' as $$
begin
  if new.ai_patient_conversation_id is not null and exists (
    select 1 from public.conversations c
    where c.id = new.ai_patient_conversation_id and c.clinic_id = new.clinic_id
      and c.ai_paused_at is not null
  ) then raise exception 'HUMAN_TAKEOVER_ACTIVE' using errcode = '42501'; end if;
  return new;
end;
$$;
drop trigger if exists trg_appointments_block_paused_ai_insert on public.appointments;
create trigger trg_appointments_block_paused_ai_insert
  before insert on public.appointments
  for each row execute function public.block_paused_ai_booking_insert();
revoke all on function public.block_paused_ai_booking_insert()
  from public, anon, authenticated, service_role;
...
-- Preserve the identity guard for every ordinary caller while allowing only
-- the reviewed approval transaction to stamp the newly linked conversation.
...
as $$
begin
  if public.ai_intake_approval_context_matches(new.clinic_id, new.id) then
    return new;
  end if;
...
-- Appointment provenance remains server-owned. The sole authenticated
-- exception is the matching human-review transaction above.
...
as $$
begin
  if public.ai_intake_approval_context_matches(
...
comment on table public.ai_patient_intakes is
  'Tenant-scoped provisional patient files collected by the AI assistant. A human approval is required before a public.patients row exists.';
comment on table public.ai_appointment_requests is
  'Pending real-slot requests for provisional AI intakes. Approval links the request to one pending public.appointments row.';
comment on column public.audit_logs.actor_type is
  'Explicit actor class. AI events use actor_id NULL and actor_type=ai; human follow-up uses the real profile id and actor_type=user.';
