...
export function ClinicSignupForm({ defaults }: { defaults: Defaults }) {
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <div className="space-y-2"><Label htmlFor="signup-phone">Clinic phone</Label><InternationalPhoneInput id="signup-phone" name="phone" value={phone} onChange={setPhone} defaultCountry={country} required disabled={pending} />{state?.fieldErrors?.phone?.[0] && <p className="text-sm text-destructive">{state.fieldErrors.phone[0]}</p>}</div>
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-2"><Label htmlFor="country">Country</Label><select id="country" name="country" value={country} onChange={(event) => setCountry(event.target.value as CountryCode)} className="h-9 w-full rounded-md border bg-transparent px-3 text-sm"><option value="KW">Kuwait</option><option value="SA">Saudi Arabia</option><option value="AE">UAE</option><option value="EG">Egypt</option></select></div>
