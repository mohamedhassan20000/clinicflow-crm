-- Production-safe metadata restore RPCs for private patient files.
-- Restore mirrors the delete RPC boundaries and keeps storage untouched.
...
begin
  ...
  if not (v_role = any (
    ...
    raise exception 'Not authorized to restore patient documents'
      using errcode = '42501';
  end if;
...
begin
  ...
  if not (v_role = any (
    ...
    raise exception 'Not authorized to restore medical note attachments'
      using errcode = '42501';
  end if;
