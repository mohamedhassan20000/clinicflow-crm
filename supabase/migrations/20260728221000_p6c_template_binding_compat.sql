-- P6C compatibility bridge: a legacy 360dialog template may have been created
-- after the provider-binding backfill but before all application instances run
-- the P6C-fixed submission path. Materialize its binding transactionally on the
-- first callback, then apply/audit the status through the same atomic boundary.
...
begin
  ...
  if not found and p_provider = 'dialog360' then
    select *
     ...
     where provider_template_id = p_provider_template_id
     for update;
