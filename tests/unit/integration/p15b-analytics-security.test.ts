...
let baseCurrentClinics=0, basePreviousClinics=0, baseCurrentUsers=0, basePreviousUsers=0;
...
beforeAll(async()=>{const {getOperatorAggregateInputs}=await import("@/lib/supabase/admin");
 const [,,baseCurrentClinicRows,basePreviousClinicRows,baseCurrentUserRows,basePreviousUserRows]=await getOperatorAggregateInputs();
 baseCurrentClinics=baseCurrentClinicRows.count??0;basePreviousClinics=basePreviousClinicRows.count??0;baseCurrentUsers=baseCurrentUserRows.count??0;basePreviousUsers=basePreviousUserRows.count??0;
 const plans=await service.from("plans").select("id, slug").in("slug",["basic","pro"]);basic=plans.data!.find(p=>p.slug==="basic")!.id;pro=plans.data!.find(p=>p.slug==="pro")!.id;
