-- Follow-up to P11R's additive reschedule boundary. AI pending-cap enforcement
-- runs before INSERT, so terminalize the locked predecessor first. The
-- replacement-chain constraint is deferred and validates the final two-sided
-- chain at commit; any later failure rolls the predecessor update back.
