...
export function ClinicSignupForm({ defaults }: { defaults: Defaults }) {
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <div className="space-y-2"><Label htmlFor="signup-phone">Clinic phone</Label><InternationalPhoneInput id="signup-phone" name="phone" value={phone} onChange={setPhone} required disabled={pending} />{state?.fieldErrors?.phone?.[0] && <p className="text-sm text-destructive">{state.fieldErrors.phone[0]}</p>}</div>
