...
import {
  issuePatientHistoryDocument,
  reprintPatientHistoryDocument,
} from "@/actions/documents";
import { Button } from "@/components/ui/button";
import { issuedDocumentDetailHref } from "@/lib/documents/module";
import type { PatientHistoryDocumentParams } from "@/lib/documents/resolvers/patient-history";

export function PatientHistoryDocumentActions({
  locale,
  params,
  idempotencyKey,
  documentId,
  ...
  function issue() {
    startIssue(async () => {
      const result = await issuePatientHistoryDocument({ ...params, locale, idempotencyKey, draftId });
      if (!result.data) {
  ...
  function reprint() {
    ...
    startReprint(async () => {
      const result = await reprintPatientHistoryDocument(documentId, params.documentType);
      if (!result.data) {
