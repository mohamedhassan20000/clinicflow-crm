...
**Status:** Approved plan — **P0, P1 (P1A–P1D), P1.5 (P1.5A–P1.5D), and P2 (P2A–P2C) are completed, reviewed, approved, and merged to `main`.** The post-P2 production-hardening work is also merged: the finalized production marketing website (PR #38), SEO production hardening (PRs #39/#40 — `docs/SEO_PRODUCTION_HARDENING.md`), UX production hardening (states/accessibility, merged in PR #40 — `docs/UX_PRODUCTION_HARDENING.md`), and the marketing CTA smooth-scroll follow-up. **P3A, P3B, P3C, and P3D are implemented on `feat/p3a-messaging-core` (uncommitted, pending review and merge — reports: `docs/reports/P3A_IMPLEMENTATION.md`, `docs/reports/P3B_IMPLEMENTATION.md`, `docs/reports/P3C_IMPLEMENTATION.md`, `docs/reports/P3D_IMPLEMENTATION.md`); P3 is feature-complete on that branch. P4–P6 have not started.**
**Date:** 2026-07-09
...
| **Total** | **~142–201** (≈ 9–13 months, single developer) | | |
...
| P4.6 | **P4.6A** | Declarative AI tool registry + aggregate analytics/financial RPCs (`SECURITY DEFINER`, clinic-scoped, small-cell suppression) + operational list/stat tools + `run_clinic_report` on `lib/reports/data.ts` cores + `ai.financial_insights` entitlement + per-user financial permission + per-tool authorization + tool-authorization tests (no UI) | P4.5A, P4.5C | 6–9 | `feat/p46a-staff-analytics-tools` |
| P4.6 | **P4.6B** | Analytics/operational/financial tools mounted into the existing staff assistant UI + entitlement/permission-gated + upgrade/permission UX | P4.6A, P4B | 4–5 | `feat/p46b-staff-analytics-ui` |
| P4.7 | **P4.7A** | Curated ar/en help corpus + `search_help` + permission-aware `get_navigation_target` + navigation registry + `staff_help` task class (no UI) | P4B, P4.5A; P4.6A (registry) | 2–3 | `feat/p47a-system-knowledge` |
| P4.7 | **P4.7B** | Server-derived capability panel (`list_my_capabilities`) + help/guidance UX in the assistant UI | P4.7A | 2–3 | `feat/p47b-capability-panel` |
...
| P7 | **P7A** | Document/template engine core: template registry, ar/en + RTL/LTR document layout primitives, clinic branding (logo/header/footer), per-document numbering/serial strategies (incl. the P3-deferred invoice serial) | P2 (RTL/i18n), P1.5 (branding) | 5–7 | `feat/p7a-document-engine` |
| P7 | **P7B** | System document templates on the engine: invoice, receipt, prescription, medical report, sick-leave, referral & lab-request, consent forms — preview/print/download; plug the rendered **invoice** document into the P3 §7.3a delivery without changing the workflow | P7A, P3D | 7–11 | `feat/p7b-system-templates` |
...
### P4.6 — Analytics, Reporting & Operational Query Assistant (10–14 days) — *added 2026-07-19; extended 2026-07-19 (operating-assistant revision); `pro_ai`-only; post-v1, non-blocking*
...
#### P4.6 execution split (2 sub-phases; merge order P4.6A → P4.6B)

**P4.6A — Tool registry, aggregate RPCs, operational tools & permission plumbing** — branch `feat/p46a-staff-analytics-tools`, est. **6–9 days**, merge **1st**, **no UI**.
...
**P4.6B — Analytics & operational tools in the staff assistant UI** — branch `feat/p46b-staff-analytics-ui`, est. **4–5 days**, merge **2nd**.
...
> **Planning status:** recommended roadmap direction only. This revision implements none of P4.7. Depends on P4B (assistant surface), P4.5A (task registry), and P4.6A (declarative tool registry). **No PHI, no new RLS surface** — the lowest-risk sub-phase in the AI track.
...
- **Capability panel (approved deliverable):** an always-accessible section of the assistant UI showing exactly what the current user is authorized to ask, **derived dynamically from the same registry resolution that mounts the model's tools** (one source of truth — no hand-maintained list to drift), supporting long lists, localized ar/en, updating with role/entitlement/permission changes.
...
- **Tests:** corpus retrieval fixtures (ar/en); navigation-honesty tests — a user whose admin hid a page never receives a link presented as accessible; capability-panel parity test — the panel's list equals the tool set actually mounted for that user in the same request context; injection fixtures with adversarial help queries; `basic`/`pro` clinics denied (standard `pro_ai` gate).
- **Acceptance:** a receptionist asks "how do I issue an invoice?" in Arabic and gets accurate steps + a working deep link; the same question about a feature their admin hid gets the honest not-enabled answer; every user's capability panel matches their true server-side tool mount; help turns are billed on the cheap task class in the usage ledger.

#### P4.7 execution split (2 sub-phases; merge order P4.7A → P4.7B)

**P4.7A — Help corpus, knowledge & navigation tools** — branch `feat/p47a-system-knowledge`, est. **2–3 days**, merge **1st**, **no UI**.
...
**P4.7B — Capability panel & guidance UX** — branch `feat/p47b-capability-panel`, est. **2–3 days**, merge **2nd**.
...
- *Acceptance:* capability-panel parity test green; panel reflects role/permission/entitlement changes without code changes.
...
### P7 — System Templates & Document Engine (12–18 days) — *added 2026-07-18*

- **Goal:** turn the founder-supplied system templates into real, production-ready, printable/deliverable clinic documents on one reusable engine — and replace P3's template-agnostic invoice payload with the rendered invoice document **without changing the event-driven delivery workflow**.
- **Why a distinct phase:** the professional invoice template was pulled out of P3 (2026-07-18 correction) because it is one instance of a broader document-engine need (receipts, prescriptions, medical reports, sick-leave, referrals, lab requests, consent forms). Building the engine once, in P7, avoids a throwaway invoice-only template in P3.
- **In scope:** a reusable **document/template engine** (template registry + typed data contracts per document type); **professional PDF & print rendering** with **clinic branding** (logo, headers, footers); **ar/en + RTL/LTR** document layout; the founder's **system templates** — invoice, receipt, prescription, medical report, sick-leave, referral & lab-request, consent forms; **preview / print / download / delivery-ready** output; and **per-document numbering/serial strategies where required** (including the **per-clinic invoice serial** deferred from P3 §7.3c — gap-free, atomic allocation). The invoice document is wired into the existing §7.3a dispatcher via the `compose summary → render message → send` seam so delivery code is untouched.
...
- **Execution split:** P7A (engine core + branding + numbering) → P7B (system templates on the engine + invoice document plugged into P3 delivery). See the execution sub-phase table.
- **Acceptance:** an issued invoice is delivered through the unchanged §7.3a workflow but now carries the rendered professional invoice document with a unique per-clinic serial; each system template renders correctly in ar/en and RTL/LTR with clinic branding; preview/print/download produce the same document; serial allocation is gap-free under concurrent issuance.
