...
### Follow-up — post-login onboarding routing and post-signup session leak

Two P1C flow defects observed with clinic `test3`:

1. **Onboarding gate skipped after login.** `signIn` only knew about `must_change_password` and returned `/dashboard` for everyone else, delegating onboarding routing to middleware — but the middleware onboarding gate is best-effort by design: it fires only when the subscription lookup resolves to *allowed* and the clinic read does not error, because `/dashboard` GET must stay a safe landing page for lapsed clinics. Any failed leg silently dropped a never-onboarded admin on `/dashboard`. Fix: `signIn` now resolves the destination deterministically — forced password change → `/change-password`; admin whose `clinics.onboarding_completed_at` is `NULL` (RLS self-read) → `/onboarding`; otherwise `/dashboard`. A clinic-lookup error logs `login_onboarding_lookup_failed` and falls back to `/dashboard` with the middleware gate as backstop; billing precedence in middleware is unchanged, and non-admin staff are never routed into the wizard. Regression coverage: `tests/unit/actions/signin-redirects.test.ts`.
2. **Stale session leaking through public signup.** After successful provisioning, `/signup/complete`'s "Go to sign in" link bounced to `/operator` when the browser still held a platform-admin session. `signUpClinic` now calls `signOut({ scope: "local" })` on the request's session after provisioning succeeds and before redirecting — clearing whatever session the browser held (operator, tenant, or an autoconfirm-minted signup session) without revoking the account's other devices, and without weakening authenticated `/login` redirects elsewhere. Email confirmation requirements are untouched. The failure-path deliberately keeps the session so the form can be retried. Regression coverage: session-clear ordering and failure-path unit tests in `p1c-signup-errors.test.ts`, plus a local-Supabase integration test where a real platform-admin session completes a token signup and ends signed out.
