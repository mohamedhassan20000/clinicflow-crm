...
- **`AssistantPageContext`** — a zod discriminated union, one variant per area: `{ type: "patient", patientId }`, `{ type: "appointments", dateRange, status?, doctorId? }`, `{ type: "revenue", dateRange, filters }`, `{ type: "reports", report, range }`, `{ type: "invoices", filter }`, `{ type: "staff" }`, `{ type: "departments" }`, `{ type: "dashboard" }`, `{ type: "doctor-schedule", dateRange }`. Validated server-side in `app/api/agent/chat/route.ts`; unknown/invalid context is dropped, not errored.
