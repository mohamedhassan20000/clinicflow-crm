...
import {
  issueRevenueReportDocument,
  reprintRevenueReportDocument,
} from "@/actions/documents";
import { Button } from "@/components/ui/button";
import { issuedDocumentDetailHref } from "@/lib/documents/module";
import type { RevenueDocumentParams } from "@/lib/documents/resolvers/revenue-report";

export function RevenueDocumentActions({
  locale,
  params,
  idempotencyKey,
  documentId,
  ...
  function issue() {
    startIssue(async () => {
      const result = await issueRevenueReportDocument({
        ...params,
        locale,
        idempotencyKey,
        draftId,
  ...
  function reprint() {
    ...
    startReprint(async () => {
      const result = await reprintRevenueReportDocument(documentId);
      if (!result.data) {
