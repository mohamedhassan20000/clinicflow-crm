  ...
  return (
    <div className="flex min-h-dvh bg-background">
        ...
        <header data-testid="dashboard-header" className="sticky top-0 z-30 flex h-16 shrink-0 items-center gap-3 border-b bg-card/95 px-4 backdrop-blur md:px-6">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="h-11 gap-2 rounded-xl px-2" aria-label="Open user menu">
      ...
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
        <SheetContent side="left" className="w-72 p-0"><SheetTitle className="sr-only">Navigation menu</SheetTitle><div className="h-full" onClick={() => setMobileOpen(false)}><Sidebar mode="sheet" items={navItems} collapsed={false} brandLabel={brandLabel} /></div></SheetContent>
      </Sheet>
    </div>
  );
}
