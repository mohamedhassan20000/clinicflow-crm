...
// Single source of truth for availability computation (§6.3). Both the
// receptionist-facing getAvailableTimeSlots action and the AI check_availability
// tool call this core, so the agent can never invent an availability answer —
// it reports exactly what this function returns. The caller supplies an already
// authorized, RLS-respecting client and the resolved clinic timezone; this
...
export async function computeAvailableSlots(params: {
  ...
  const { supabase, clinicId, doctorId, dateIso, timeZone } = params;
  ...
  const dayStartIso = fromZonedTime(`${dateIso}T00:00:00.000`, timeZone).toISOString();
  const dayEndIso = fromZonedTime(`${dateIso}T23:59:59.999`, timeZone).toISOString();
  ...
  for (let t = overallStart; t < overallEnd; t += STEP) {
    ...
    if (!inWindow && !inBreak) continue; // Outside all windows and not a break gap
